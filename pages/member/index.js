...
const Index = ({ memberData, title, description }) => {
  ...
  const memberListItems = memberRealData.map((memberListItem) => memberListItem.data);
...
export async function getStaticProps() {
  ...
  const memberData = members.map((member) => {
    ...
    const memberEditedContent = memberRawContent.slice(0, 3) + "\nslug: " + { member }.member.slice(0, -3) + "\n" + memberRawContent.slice(3);
